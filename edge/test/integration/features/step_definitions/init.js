...
var environment = config['devops_sf-cicd_sf-srinivas_sf'].env;
...
console.log('&$devops_sf-cicd_sf-srinivas_sf api: [' +  domains[environment] + ', ' + config['devops_sf-cicd_sf-srinivas_sf'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['devops_sf-cicd_sf-srinivas_sf'].app, config['devops_sf-cicd_sf-srinivas_sf'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['devops_sf-cicd_sf-srinivas_sf'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
